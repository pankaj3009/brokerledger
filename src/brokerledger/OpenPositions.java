...
public class OpenPositions {
    ...
    /**
     * Creates openPositions for next day, if available.Else returns with the
     * last positionClosingDate.
     *
    ...
    public OpenPositions(ArrayList<Position> openingPosition, ArrayList<Position> netPosition, ArrayList<Trade> trades, HashMap<String, SymbolMapping> mapping, Date lastProcessedDate, double ledgerBalance, double ytdMTM, Date endDate) {
        ...
        for (Position p : this.netPosition) {
            ...
            if (p.positionSize != 0 && (s.expiry != null && Utilities.dateCompare(s.expiry, positionDate, "ddddMMyy") > 0)) {
                Utilities.writeToFile(Utilities.getDateString(positionClosingDate, "yyyyMMdd") + "_" + closingPositionFileName, p.brokerSymbol + "," + p.positionSize + "," + p.positionEntryPrice + "," + p.positionMTMPrice + "," + Utilities.getDateString(positionClosingDate, "yyyyMMdd") + "," + p.cost);
            }
        }
    }
